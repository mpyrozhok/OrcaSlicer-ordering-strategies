...
std::vector<const PrintInstance*> chain_print_object_instances_spiral(const std::vector<const PrintObject*>& print_objects, const Point* start_near)
{
    ...
    // Sort indices by distance from centroid, then by angle within distance bands.
    ...
    std::sort(indices.begin(), indices.end(), [&](size_t a, size_t b) {
        ...
        double dist2_a = va.squaredNorm();
        double dist2_b = vb.squaredNorm();
        // Group by distance bands (10% tolerance), then sort by angle within band.
        double mean_dist2 = (dist2_a + dist2_b) / 2.0;
        if (mean_dist2 > 0 && std::abs(dist2_a - dist2_b) / mean_dist2 < 0.1) {
            // Same distance band: sort by angle.
        ...
        // Different distance bands: closer first.
